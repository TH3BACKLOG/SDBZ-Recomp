        ...
        tc.Run("GS T4HL/T4HH shared-plane upload preserves both index planes via RMW", [](TestCase &t)
        {
            ...
            for (uint32_t y = 0; y < kRrh; ++y)
            {
                for (uint32_t x = 0; x < kRrw; ++x)
                {
                    ...
                    const uint32_t memA = GSMem::ReadPixelP4HL(vram.data(), kDbp, kDbw, x, y);
                    const uint32_t memB = GSMem::ReadPixelP4HH(vram.data(), kDbp, kDbw, x, y);
            ...
            for (uint32_t y = 0; y < kRrh; ++y)
            {
                for (uint32_t x = 0; x < kRrw; ++x)
                {
                    ...
                    const uint32_t memByte = GSMem::ReadPixelP8H(vram.data(), kDbpT8H, kDbwT8H, x, y);
                    if (memByte != expected)
